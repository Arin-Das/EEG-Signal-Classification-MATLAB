classdef EEGAppBands < matlab.apps.AppBase
    ...
    properties (Access = private)
        ...
        % Conventional 19-electrode 10-20 ordering, used only for a
        % headerless CSV with exactly 19 signal columns. A CSV header always
        % takes priority, so custom montages retain their original labels.
        Default19ChannelNames = {'Fp1','Fp2','F7','F3','Fz','F4','F8', ...
    ...
    methods (Access = private)
        ...
        function [data,fsOut,chNamesOut] = readAnyEEGFile(app,file,needsFs)
            ...
            switch lower(ext)
                ...
                case '.csv'
                    [data,fsOut,chNamesOut] = app.readCsvWithAutoSamplingRate(file,needsFs);
        ...
        function [data,fsOut,chNamesOut] = readCsvWithAutoSamplingRate(app,file,needsFs)
            ...
            try
                ...
                names = cellstr(opts.VariableNames);
                % Use a real CSV header as the channel list. Headerless
                % numeric files receive MATLAB's default Var1, Var2, ...
                % names, which must not replace the app's normal fallback.
                isDefaultNames = all(cellfun(@(s) ~isempty(regexp(s,'^Var\d+$','once')), names));
                if ~isDefaultNames && numel(names) == size(data,2)
                    chNamesOut = strtrim(names(:)');
                end
                for c = 1:min(numel(names),size(data,2))
                    label = lower(regexprep(names{c},'[^a-z0-9]',''));
                    if contains(label,'timestamp') || strcmp(label,'time') || contains(label,'time')
                        timeCol = c;
            ...
            if ~isempty(timeCol)
                ...
                % A labelled time column is metadata, not an EEG channel;
                % remove it even if its interval cannot yield a valid fs.
                data(:,timeCol) = [];
                if numel(chNamesOut) >= timeCol
                    chNamesOut(timeCol) = [];
                end
            end
        end
        ...
        function loadEEG(app,file)
            ...
            % "channelNames" variable) > standard fallback montage when
            % the column count matches > generic Ch1..ChN.
            ...
            elseif nChData == numel(app.Default19ChannelNames)
                app.ChannelNames = app.Default19ChannelNames;
            else
